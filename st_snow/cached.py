...
STSTATE_SNOWFLAKE_CONNECTION="stsnow_conn"
STSTATE_SNOWFLAKE_RESULTS="stsnowr_esults"
...
class SnowCacheSession(Session):
    class SessionBuilder(Session.SessionBuilder):
        def _create_internal(self, conn: Optional[SnowflakeConnection] = None) -> "Session":
            new_session = SnowCacheSession(
                SnowCacheServerConnection({}, conn) if conn else SnowCacheConnection(self._options)
            )
