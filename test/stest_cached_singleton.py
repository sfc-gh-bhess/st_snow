...
def getOrRegetSession(fname = "/tmp/snowflake/creds.json") -> Session:
    ...
    snowcreds[st_snow.cached.cached.SnowCacheCursor.CACHEKEY_TTL] = 30
...
st.markdown(f"session: {session2}")
...
st.markdown(f"session: {session3}")
